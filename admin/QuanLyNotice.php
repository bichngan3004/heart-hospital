    ...
    <section>
        <div class="col-md-6" style="margin-left: 35px; width:95%">
            <form method="post" action="" enctype="multipart/form-data" id="form8" name="form8">
                <h3 style="text-align: center;font-weight: bold">QUẢN LÝ NOTICE</h3>
                <table class="pt-5" style="border-left:1px solid #ccc; border-right:1px solid #ccc;" cellpadding="5" cellspacing="0">
                    <tr>
                        ...
                        <td style="text-align:left"><input style="border: 2px ridge #efefef; border-radius:5px" type="text" name="txttieude" id="txttieude" value="<?php echo $p->laycot("select tieude from notice where id_notice='$layid'"); ?>" />
                            <label for="tin"></label>
                    ...
                    <tr>
                        ...
                        <td colspan="3" >
                            ...
                            <textarea name="txtnoidung" id="txtnoidung">
                                <?php echo $p->laycot("select noidung from notice where id_notice='$layid'"); ?>
                            </textarea>
                        </td>
                    ...
                    <tr>
                        ...
                        <td colspan="3" style="text-align:left">
                            ...
                            <input type="file" name="myfile" id="myfile" value="<?php echo $p->laycot("select hinhanh from notice where id_notice='$layid'"); ?>" />
                        </td>
                    </tr>
                ...
                <div class="information">
                    <div class="container" style="width:100%;">
                        <h3 style="text-align: center;font-weight: bold">DANH SÁCH NOTICE</h3>
                        
                        <?php $p->loadlistnotice("SELECT * FROM notice order by id_notice asc"); ?>
                    </div>
                </div>
        </div>
        </form>
...
<button onclick="topFunction()" id="myBtn" title="Go to top"><i class="fa fa-angle-double-up"></i></button>
    ...
    if (isset($_POST['them'])) {
        ...
        if ($name != '') {
            ...
            if ($p->myupfile($name, $tmp_name, "../img/news/") == 1) {
                $sql = "INSERT INTO notice(tieude,noidung,hinhanh,ngaydang) VALUES ('$tieude', '$noidung', '$name', current_timestamp())";
                if (mysqli_query($p->connect(), $sql) == true) {
                    header("location:QuanLyNotice.php");
                } else {
                    echo "<script>alert('Thêm notice không thành công!')</script>";
                }
            } else {
    ...
    if (isset($_POST['update'])) {
        ...
        $type = $_FILES['myfile']['type'];
        if ($idsua > 0) {
            $sql = "UPDATE notice SET tieude = '$tieude', hinhanh = '$name', noidung = '$noidung' WHERE id_notice = '$idsua' limit 1";
            if (mysqli_query($p->connect(), $sql) == true) {
                header("location:QuanLyNotice.php");
            } 
            else {
            ...
            echo "<script>alert('Vui lòng chọn notice cần sửa!')</script>";
        }
    }
    //Xóa tin tức
    if (isset($_POST['xoa'])) {
        ...
        if ($idxoa > 0) {
            if ($p->themxoasua("DELETE FROM notice WHERE id_notice='$idxoa'") == 1) {
                header("location:QuanLyNotice.php");
            } else {
                echo '<script>alert("Xoa notice không thành công")</script>';
            }
        } else {
            echo '<script>alert("Vui lòng chọn notice cần xóa")</script>';
        }
    }
    ?>
